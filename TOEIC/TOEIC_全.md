我目前的多益閱讀能力約為 500 分，整體成績約 500 分上下，目標是透過每天 30 分鐘、持續 90 天的學習計畫，將成績提升至中高分區間。我會每日記錄學習進度，並以 VS Code 的 Markdown 檔案格式傳送給你，以確保學習內容保持同步。當我完成某一日的學習後，會請你提供下一日的教學內容（例如完成第 5 天後，請提供第 6 天內容）。

學習目標為：掌握 2,500 個核心多益單字、強化文法結構與閱讀理解技巧，**專注於閱讀測驗與答題策略**，**不包含聽力練習**，以便在閱讀分數方面取得顯著進步。

目前前 41 天的學習內容已涵蓋初級至中級文法與詞彙，並已完成如下進度：
- 累積詞彙超過 1,000+，涵蓋商務、財經、行銷、科技主題
- 文法主題包含完成式、被動語態、假設語氣、倒裝句、關係子句、動名詞與不定詞等
- 閱讀練習包含 200～300 字文章與細節/推論題

因此，從第 42 天起，請依據以下原則安排教學內容：

1. **單字難度再提升**：請避免與前日重複，並加入如 business idioms、marketing phrases、財經動詞、會議/報表常見表達等中高頻詞彙。
2. **文法難度加深**：請依序加入「名詞子句、分詞構句、比較級與倍數表達、時間條件省略句」等進階文法結構。
3. **閱讀篇章實戰化**：請使用商務電郵、內部備忘錄、通知、提案等文體，每篇約 250 字，並附上 3～5 題推論、細節、文意邏輯題（如主旨判斷、句子插入、順序合理性等）。
4. **格式固定不變**：每篇教學皆使用六個反引號包覆全體 Markdown 區塊，內部使用四個反引號標示語法（以避免跑版），並與前 41 天內容保持一致風格。

請特別注意：
- **單字與文法每次不得重複**。
- **每日學習包需包含：單字表（25 個）、文法說明＋練習（5 題）、閱讀理解（200～300 字＋5 題）**。
- **必要時可適度提升難度**，但請先評估學習曲線，不宜超出理解門檻。
